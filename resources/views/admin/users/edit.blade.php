...
@section('content')
    ...
    <form action="{{route('admin.users.update', $user)}}"
          method="post"> {{--Verstuurt gegevens naar admin.users.update--}}
        @csrf
        ...
        <input type="text" id="naam" name="name"
               value="{{old('name', $user->name)}}"><br> {{--Lees: toon eerst de vorige ingevulde waarde, lukt het niet? Gebruik dan de waarden van de database.--}}
        ...
        <button type="submit" class="btn btn-edit">Gebruiker aanpassen</button>
    </form>
@endsection
